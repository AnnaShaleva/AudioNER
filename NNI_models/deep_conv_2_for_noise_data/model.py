...
LOG = logging.getLogger('deep_conv_2_for_noise_data_model_keras')
...
def build_model(hyper_params, input_shape=(const.HEIGHT, const.LENGTH, 1), num_classes=const.N_CLASSES, num_subclasses=const.N_SUBCLASSES):
    ...
    conv1 = Conv2D(hyper_params['conv1_size'], kernel_size=(8, 10), use_bias=True, padding='same', activation='relu')(pool1)
    pool2 = MaxPooling2D((2, 2), padding='same')(conv1)
    conv2 = Conv2D(hyper_params['conv2_size'], kernel_size=(4, 10), use_bias=True, padding='same', activation='relu')(pool2)
    pool3 = MaxPooling2D((2, 2), padding='same')(conv2)
    conv3 = Conv2D(hyper_params['conv3_size'], kernel_size=(4, 10), use_bias=True, padding='same', activation='relu')(pool3)
    pool4 = MaxPooling2D((2, 2), padding='same')(conv3)

    conv4 = Conv2D(hyper_params['conv4_size'], kernel_size=(2, 5), use_bias=True, padding='same', activation='relu')(pool4)
    pool5 = MaxPooling2D((2, 2), padding='same')(conv4)
    conv5 = Conv2D(hyper_params['conv5_size'], kernel_size=(2, 5), use_bias=True, padding='same', activation='relu')(pool5)
    pool6 = MaxPooling2D((2, 2), padding='same')(conv5)
    conv6 = Conv2D(hyper_params['conv6_size'], kernel_size=(1, 1), use_bias=True, padding='same', activation='relu')(pool6)
    pool7 = MaxPooling2D((2, 2), padding='same')(conv6)
    conv7 = Conv2D(hyper_params['conv7_size'], kernel_size=(1, 1), use_bias=True, padding='same', activation='relu')(pool7)
    pool8 = MaxPooling2D((2, 2), padding='same')(conv7)

    flatten = Flatten()(pool8)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument("--dataset_name", type=str,default='test_noised_dataset', help='Dataset name', required=False)
