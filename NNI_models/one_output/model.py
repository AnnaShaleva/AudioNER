...
LOG = logging.getLogger('single_output_model_keras')
...
def build_model(hyper_params, input_shape=(const.HEIGHT, const.LENGTH, 1), num_classes=const.N_CLASSES, num_subclasses=const.N_SUBCLASSES):
    ...
    model = Model(inputs=visible, outputs=subclasses_output)
...
def train(args, params):
    ...
    print('Fitting model...')
    results = model.fit(X_train, Y2_train, epochs=args.epochs, verbose=1,
                        validation_data=(X_test, Y2_test), callbacks=[SendMetrics(), TensorBoard(log_dir=TENSORBOARD_DIR)])

    _, acc = model.evaluate(X_test,Y2_test, verbose=1)
...
if __name__ == '__main__':
    ...
    PARSER.add_argument("--dataset_name", type=str, default='one_output_dataset', help='Dataset name', required=False)
    PARSER.add_argument("--epochs", type=int, default=700, help="Train epochs", required=False)
