...
LOG = logging.getLogger('deep_conv_spectrogram_model_keras')
TENSORBOARD_DIR = os.environ['NNI_OUTPUT_DIR']


def load_dataset(dataset_name, part_train):
    print('Load data...')
    X_train, X_spectrogram_train, Y1_train, Y2_train, X_test, X_spectrogram_test, Y1_test, Y2_test = get_test_and_train_data(dataset_name, part_train)
    ...
    return X_train, X_spectrogram_train, Y1_train, Y2_train, X_test, X_spectrogram_test, Y1_test, Y2_test


def build_model(hyper_params, input_shape=(129, 57, 1), num_classes=const.N_CLASSES, num_subclasses=const.N_SUBCLASSES):
    print('Building model...')
    #input layer
    visible = Input(shape=input_shape)
    conv0 = Conv2D(hyper_params['conv0_size'], kernel_size=(10, 8), use_bias=True, padding='same', activation='relu')(visible)

    #classification layers
    pool1 = MaxPooling2D((2, 2), padding='same')(conv0)
    conv1 = Conv2D(hyper_params['conv1_size'], kernel_size=(10, 4), use_bias=True, padding='same', activation='relu')(pool1)
    pool2 = MaxPooling2D((2, 2), padding='same')(conv1)
    conv2 = Conv2D(hyper_params['conv2_size'], kernel_size=(5, 2), use_bias=True, padding='same', activation='relu')(pool2)
...
def train(args, params):
    ...
    X_train, X_spectrogram_train,  Y1_train, Y2_train, X_test, X_spectrogram_test, Y1_test, Y2_test = load_dataset(args.dataset_name, args.num_train)

    print('Fitting model...')
    results = model.fit(X_spectrogram_train, [Y1_train, Y2_train], epochs=args.epochs, verbose=1,
                        validation_data=(X_spectrogram_test, [Y1_test, Y2_test]), callbacks=[SendMetrics(), TensorBoard(log_dir=TENSORBOARD_DIR)])

    _, _, _, cat_acc, subcat_acc = model.evaluate(X_spectrogram_test, [Y1_test, Y2_test], verbose=0)
